...
void WinMainCRTStartup()
{
	...
	// Transmit the command line to the previous instance, if any
	{
		...
		if (hwnd) {
			COPYDATASTRUCT cds;
			cds.dwData = bUnicode;
			cds.cbData = getStringSize(pszCmdLine);
			...
			Cancel:
				...
				if (id == IDCCMD_LANGUAGE) {
					...
					if (!IsKeyDown(VK_SHIFT) && IDNO ==
							messageBox(s_hdlgMain, ASK_QUIT, MB_YESNO | MB_DEFBUTTON2 | MB_ICONQUESTION)) {
						goto Cancel;
					}
				}
				...
				setAutoStartEnabled(ToBool(IsDlgButtonChecked(s_hdlgMain, IDCCHK_AUTOSTART)));
			}
...
INT_PTR CALLBACK prcAbout(HWND hDlg, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	switch (uMsg) {
		...
		case WM_CTLCOLORSTATIC:
			{
				...
				if (id == IDCLBL_EMAIL || id == IDCLBL_WEBSITE) {
					HFONT hFont = (HFONT)GetWindowLong(hDlg, DWLP_USER);
					if (!hFont) {
						LOGFONT lf;
						GetObject((HFONT)SendMessage((HWND)lParam, WM_GETFONT, 0,0), sizeof(lf), &lf);
						lf.lfUnderline = TRUE;
						hFont = CreateFontIndirect(&lf);
						SetWindowLong(hDlg, DWLP_USER, (LONG)hFont);
					}
		...
		case WM_COMMAND:
			DeleteFont((HFONT)GetWindowLong(hDlg, DWLP_USER));
			EndDialog(hDlg, IDOK);
			break;
	}
...
bool Shortcut::tryChangeDirectory(HWND hdlg, LPCTSTR pszDirectory)
{
	...
	if (findVisibleChildWindow(hdlg, _T("SHBrowseForFolder"), true)) {
		...
		const HANDLE hProcess = OpenProcess(PROCESS_VM_OPERATION | PROCESS_VM_WRITE,
			FALSE, idProcess);
		if (hProcess) {
			const DWORD size = getStringSize(pszDirectoryNoQuotes);
