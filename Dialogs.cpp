...
PLACEMENT main_dialog_placements[] = {
	{ .id = IDCLST, .flags = kPlacementWidth | kPlacementHeight },
	
	{ .id = IDCCMD_ADD, .flags = kPlacementTop },
	{ .id = IDCCMD_DELETE, .flags = kPlacementTop },
	{ .id = IDCCMD_EDIT, .flags = kPlacementTop },
	{ .id = IDCLBL_HELP, .flags = kPlacementTop },
	{ .id = IDCLBL_SHORTCUTSCOUNT, .flags = kPlacementTop },
	{ .id = IDCTXT_SHORTCUTSCOUNT, .flags = kPlacementTop },
	{ .id = IDCCHK_AUTOSTART, .flags = kPlacementTop },
	
	{ .id = IDCOPT_TEXT, .flags = kPlacementTop },
	{ .id = IDCFRA_TEXT, .flags = kPlacementTop | kPlacementWidthHalf },
	{ .id = IDCTXT_TEXT, .flags = kPlacementTop | kPlacementWidthHalf },
	{ .id = IDCCMD_TEXT_MENU, .flags = kPlacementTop | kPlacementLeftHalf },
	
	{ .id = IDCOPT_COMMAND, .flags = kPlacementTop | kPlacementLeftHalf },
	{ .id = IDCFRA_COMMAND, .flags = kPlacementTop | kPlacementLeftHalf | kPlacementWidthHalf },
	{ .id = IDCTXT_COMMAND, .flags = kPlacementTop | kPlacementLeftHalf | kPlacementWidthHalf },
	{ .id = IDCCMD_COMMAND, .flags = kPlacementTop | kPlacementLeft },
	{ .id = IDCLBL_ICON, .flags = kPlacementTop | kPlacementLeftHalf },
	{ .id = IDCCMD_CMDSETTINGS, .flags = kPlacementTop | kPlacementLeftHalf },
	{ .id = IDCCMD_TEST, .flags = kPlacementTop | kPlacementLeftHalf },
	
	{ .id = IDCLBL_PROGRAMS, .flags = kPlacementTop },
	{ .id = IDCCBO_PROGRAMS, .flags = kPlacementTop },
	{ .id = IDCTXT_PROGRAMS, .flags = kPlacementTop | kPlacementWidth },
	{ .id = IDCIMG_PROGRAMS, .flags = kPlacementTop | kPlacementLeft },
	
	{ .id = IDCLBL_DESCRIPTION, .flags = kPlacementTop },
	{ .id = IDCTXT_DESCRIPTION, .flags = kPlacementTop | kPlacementWidth },
	
	{ .id = IDCLBL_DONATE, .flags = kPlacementTop },
	
	{ .id = IDCCMD_HELP, .flags = kPlacementTop | kPlacementLeft },
	{ .id = IDCCMD_COPYLIST, .flags = kPlacementTop | kPlacementLeft },
	{ .id = IDCCMD_LANGUAGE, .flags = kPlacementTop | kPlacementLeft },
	{ .id = IDCCMD_ABOUT, .flags = kPlacementTop | kPlacementLeft },
	{ .id = IDCCMD_QUIT, .flags = kPlacementTop | kPlacementLeft },
	{ .id = IDCANCEL, .flags = kPlacementTop | kPlacementLeft },
	{ .id = IDOK, .flags = kPlacementTop | kPlacementLeft },
};
...
BaseMenuItem* BaseMenuItem::findItemAndCleanMenu(HMENU menu, UINT id) {
	MENUITEMINFO mii = {
		.cbSize = sizeof(mii),
		.fMask = MIIM_FTYPE | MIIM_DATA | MIIM_ID | MIIM_SUBMENU,
	};
	
	BaseMenuItem *item_to_execute = nullptr;
	for (UINT pos = 0; GetMenuItemInfo(menu, pos, /* fByPosition= */ true, &mii); pos++) {
		auto *item = reinterpret_cast<BaseMenuItem*>(mii.dwItemData);
		if (item) {
			if (mii.wID == id && id) {
...
void FileMenuItem::populate(HMENU menu) {
	...
	MENUITEMINFO mii_get_data = {
		.cbSize = sizeof(mii_get_data),
		.fMask = MIIM_DATA,
	};
	
	// For inserting real items.
	MENUITEMINFO mii_real = {
		.cbSize = sizeof(mii_real),
		.fMask = MIIM_BITMAP | MIIM_STRING,
		.hSubMenu = NULL,
	};
...
void AppMenuItem::populate(HMENU menu) {
	MENUITEMINFO mii = {
		.cbSize = sizeof(mii),
		.fMask = MIIM_BITMAP | MIIM_DATA | MIIM_STRING | MIIM_ID,
...
void onMainCommand(UINT id, WORD notify, HWND hwnd) {
	switch (id) {
		...
		case IDCCMD_ADD: {
			...
			const UINT selected_cmd_id = UINT(TrackPopupMenu(
				menu,
		...
		case IDCCMD_DELETE: {
			...
			auto *sh = reinterpret_cast<Shortcut*>(lvi.lParam);
			sh->unregisterHotKey();
			delete sh;
		...
		default:
			if (ID_ADD_SPECIALCHAR_FIRST <= id && id < ID_ADD_ITEM_FIRST) {
				Shortcut *const shortcut = new Shortcut;
				const TCHAR text[] = { TCHAR(id - ID_ADD_SPECIALCHAR_FIRST), _T('\0') };
				...
				const TCHAR text[] = { TCHAR(id - ID_TEXT_SPECIALCHAR_FIRST), _T('\0') };
				String command;
...
INT_PTR CALLBACK prcMain(HWND hdlg, UINT message, WPARAM wParam, LPARAM lParam) {
	...
	switch (message) {
		...
		case WM_INITDIALOG: {
			...
			min_size = {
				.cx = orig_client_rect.right - orig_client_rect.left,
				.cy = orig_client_rect.bottom - orig_client_rect.top,
			};
			GetClientRect(hdlg, &orig_client_rect);
			
			// Get the initial position of controls
			for (auto& placement : main_dialog_placements) {
				placement.hctl = GetDlgItem(hdlg, placement.id);
				RECT ctl_rect;
				GetWindowRect(placement.hctl, &ctl_rect);
				placement.size = {
					.cx = ctl_rect.right - ctl_rect.left,
					.cy = ctl_rect.bottom - ctl_rect.top,
				};
			...
			LVCOLUMN lvc = { .mask = LVCF_FMT | LVCF_TEXT | LVCF_SUBITEM };
			...
			for (lvi.iItem = 0; lvi.iItem < shortcut_count ; lvi.iItem++) {
				ListView_GetItem(s_hwnd_list, &lvi);
				auto *sh = reinterpret_cast<Shortcut*>(lvi.lParam);
				if (sh->m_type == Shortcut::Type::kCommand) {
					sh->fillGetFileIcon(apgfi[--shortcut_icon_count] = new GETFILEICON, /* small_icon= */ true);
				}
			}
		...
		case WM_SIZE: {
			...
			GetWindowPlacement(hdlg, &dialog_wp);
			e_main_dialog_size = {
				.cx = dialog_wp.rcNormalPosition.right - dialog_wp.rcNormalPosition.left,
				.cy = dialog_wp.rcNormalPosition.bottom - dialog_wp.rcNormalPosition.top,
			};
		...
		case WM_GETFILEICON:
			if (lParam) {
				auto *pgfi = reinterpret_cast<GETFILEICON*>(lParam);
...
INT_PTR CALLBACK prcCmdSettings(HWND hdlg, UINT message, WPARAM wParam, LPARAM UNUSED(lParam)) {
	switch (message) {
		case WM_INITDIALOG: {
			...
			SendDlgItemMessage(hdlg, IDCCBO_SHOW, CB_SETCURSEL, WPARAM(show_option_index), 0);
			return true;
		}
	...
	switch (message) {
		case WM_INITDIALOG: {
			...
			for (int lang = 0; lang < i18n::kLangCount; lang++) {
				ListBox_AddString(list_hwnd, getLanguageName(i18n::Language(lang)));
			}
		...
		case WM_COMMAND:
			switch (LOWORD(wParam)) {
				...
				// Fall-through
				case IDOK: {
					const i18n::Language lang = i18n::Language(ListBox_GetCurSel(list_hwnd));
...
void updateList() {
	...
	static constexpr UINT kControlIds[] = {
...
int addItem(Shortcut* shortcut, bool selected) {
	LVITEM lvi = {
		...
		.iItem = 0,
		.iSubItem = 0,
		.state = selected ? LVIS_SELECTED | LVIS_FOCUSED : 0U,
		.stateMask = LVIS_SELECTED | LVIS_FOCUSED,
		.pszText = LPSTR_TEXTCALLBACK,
		.iImage = I_IMAGECALLBACK,
		.lParam = reinterpret_cast<LPARAM>(shortcut),
	};
...
void onShortcutsCountChanged() {
	SetDlgItemInt(
		...
		UINT(ListView_GetItemCount(s_hwnd_list)), /* bSigned= */ true);
}


Shortcut* getSelectedShortcut() {
	LVITEM lvi = { .iItem = ListView_GetNextItem(s_hwnd_list, -1, LVNI_SELECTED) };
	if (lvi.iItem < 0) {
		return nullptr;
	}
	lvi.mask = LVIF_PARAM;
...
void Shortcut::onGetFileInfo(dialogs::GETFILEICON& gfi) {
	if (gfi.flags & SHGFI_SYSICONINDEX) {
		...
		LVFINDINFO lvfi = {
			.flags = LVFI_PARAM,
			.lParam = reinterpret_cast<LPARAM>(gfi.shortcut),
		};
	...
	switch (column_text) {
		...
		case kColCond: {
			for (int cond = 0; cond < Keystroke::kConditionCount; cond++) {
				TCHAR cond_chr = Keystroke::kConditionChars[int(m_conditions[cond])];
				if (cond_chr) {
					output += cond_chr;
	...
	switch (s_sort_column) {
		case kColContents:
			if (shortcut1->m_type != shortcut2->m_type) {
				...
				return int(shortcut1->m_type) - int(shortcut2->m_type);
			}
			break;
